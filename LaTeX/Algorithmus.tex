...
\begin{alignat*}{2}
&\text{(i)}\quad&&|E_{C_1,\neg T}|<|E_{\bar{C},\neg T}|\\
&\text{(ii)}\quad&&c(C_1)=c(\bar{C})-c(\tilde{C})-\sum_{i=2}^{c}c(C_i)\leq c(\bar{C})<0
\end{alignat*}
...
\end{defn}
\begin{anm}Zirkulationen sind an sich für beliebige gerichtete Graphen mit einer Kapazitätsfunktion definiert, für unsere Zwecke genügt die obige Version. Die Notation für den Graph der durchflossenen Kanten überträgt sich.\end{anm}
...
\cref{zirkI} ist also gegeben. Damit bleibt zu zeigen, dass sich der Fluss von $z$ für alle Knoten $v\in V(\bar{G})$ gemäß \cref{zirkII} ausgleicht:
