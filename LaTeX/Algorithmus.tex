...
\section{Vorbetrachtungen}\label{ch:BL}
...
Wir werden nun auf den Beweis von \cref{opt} hinarbeiten. Dann haben wir gezeigt, dass der Algorithmus eine optimale Lösung des Transportproblems gefunden hat, wenn Phase 2 in Ermangelung einer geeigneten Kante $e$ beendet wird. \cref{iterierbar} ist allgemeiner formuliert als notwendig, da wir in \cref{ch:alg2} darauf zurückgreifen werden.
...
In beiden Fällen definieren wir uns die Zirkulation $z_{n+1}$ mit $z_{n+1}(e)=\tilde{z}(e)+1$ für alle $e\in E(C)$ und $z_{n+1}=0$ sonst. Offensichtlich ist $\bar{G}_{z_{n+1}}=C$ ein gerichteter Kreis. Wir setzen $\mathscr{Z}:=\mathscr{Z}'\backslash\tilde{z}\cup z_{n+1}$, womit für $z$ und $\mathscr{Z}$ nach Konstruktion gilt:
TODO HIER!
\begin{alignat*}{1}
\forall e\in E(\bar{G})\backslash E(C)\colon\quad &\sum_{z_i\in\mathscr{Z}} z_i(e)=\sum_{z'_i\in\mathscr{Z'}}z'_i(e)+\tilde{z}(e)-z_{n+1}(e)=\sum_{z'_i\in\mathscr{Z'}}z'_i(e)=z'(e)=z(e)\\
\forall e\in E(C)\colon\quad &z(e)=1+z'(e)=1+\sum_{z'_i\in\mathscr{Z'}} z'_i(e)=\sum_{z_i\in\mathscr{Z}} z_i(e)\\
&c(z)=c(C)+c(z')=c(C)+\sum_{z'_i\in\mathscr{Z'}} c(z'_i) =\sum_{z_i\in\mathscr{Z}} c(z_i)\end{alignat*}
...
\begin{lem}Seien $f$ und $\hat{f}$ zwei maximale Flüsse auf einem Netzwerk $(G,b,c,u)$. Dann ist $z:=\hat{f}-f$ eine Zirkulation auf $R_{N,f}$ und für die Kosten gilt $c(z)=c(\hat{f})-c(f)$.\end{lem}
\begin{proof}Sei $E:=E(G)$ die Menge aller Kanten des Graphen und $\bar{E}:=\varphi(E)$ die Menge ihrer Residualkanten in $\bar{G}$. Zunächst betrachten wir die Kosten von $z$:
...
Sei $N=(G,b,c,u)$ ein Netzwerk mit einer optimalen zulässigen Baumlösung $(\hat{T},\hat{f})$ und einer zulässigen Baumlösung $(T,f)$, sodass $c(f)>c(\hat{f})$. Dann ist $z:=\hat{f}-f$ eine Zirkulation und nach \cref{zerl} sind ihre Kosten $c(z)=c(\hat{f})-c(f)<0$ negativ. Des Weiteren BLABLA zerfallen für eine Menge $\mathscr{C}=\{C_1,\ldots,C_n\}$ mit Faktoren $\lambda_1,\ldots,\lambda_n$. \Obda seien alle $\lambda_i\neq 0$.

Da $c(z)<0$ ist, gibt es mindestens einen negativen Kreis $C_j\in\mathscr{C}$. Sei $e\in E(C_j)$ eine Kante dieses Kreises. Ist $e$ eine Residualkante, so gilt $\hat{f}(\varphi(e))-f(\varphi(e))<0$, woraus $f(e)>0$ und damit $\varphi(e)\in E(T)$ folgt. Ansonsten ist $e\in E(G)$ und vom Algorithmus wählbar. Wir können also \cref{iterierbar} anwenden, das Theorem folgt.\end{proof}
...
\cite[Cunningham, 1976]{cycling} führte eine Methode ein, mit der durch eine geschickte Wahl der entfernten Kante Cycling verhindert werden kann, ohne dass die Auswahl der hinzufügbaren Kanten eingeschränkt wird. Dafür benötigen wir folgende Definition:
