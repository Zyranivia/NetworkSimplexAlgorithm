...
\draw (v0) to node[left] {$e_1$} (v1);
\draw (v2) to node[right] {$e_2$}(v3);

\begin{scope}[on background layer]
  \clip (0,-0.5) rectangle (4,2);
  \draw[dashed] (0.5,0.75) circle(1);
  \draw[dashed] (3.5,0.75) circle(1);
\end{scope}
...
\draw (v4) to node[left] {$e_1$} (v5);
\draw[dashed] (v5) to [bend left] (v7);
\draw[dashed] (v7) to (v6);
\draw[dashed] (v6) to [bend left] (v4);
\draw (v8) to node[right] {$e_2$} (v9);
\draw[dashed] (v8) to [bend left] (v6);
\draw[dashed] (v7) to [bend left] (v9);
\end{tikzpicture}
\end{document}