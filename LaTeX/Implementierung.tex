...
Die Funktionen \lstinline|deleteEdge| und \lstinline|deleteNode| schlagen fehl, wenn auf den zu löschenden Kanten noch Fluss ist. So kann unter anderem durch Löschen des künstlichen Knotens überprüft werden, ob die Instanz lösbar ist. Für Phase 1 des Algorithmus (siehe \cref{HCLC}) nutzen wir die Funktion \lstinline|addFlow|, die einen Weg von einer Quelle zu einer Senke verlangt.
...
Sei $G$ ein gerichteter Graph mit $n$ Knoten sowie $m$ Kanten und $T$ ein aufspannender Baum von $G$. In der Herleitung des Algorithmus haben wir genutzt, dass für eine Kante $e\in E(G)\backslash E(T)$ außerhalb des Baumes der Kreis $C_{T,e}$ eindeutig ist. Ein Kreis $C_{T,e}$ lässt sich beispielsweise per Tiefensuche in Laufzeit $\bigO(n)$ berechnen, womit wir pro Iteration bei einer Komplexität von $\bigO(n\cdot(m-n)\cdot \text{poly}(n,m))$\footnotemark{} für Laufzeit $\text{poly}(n,m)$ des Pivotalgorithmus sind.
...
Nicht explizit in der Klassendeklaration schlägt sich die Invariante nieder, dass im Verlauf des Algorithmus jeder \lstinline|Circle| durch die erste Kante als einzige Nichtbaumkante spezifiziert wird.
...
Der nächste Abschnitt behandelt nun, wie wir gemäß \cref{ch:deg} eine Kante $e'$ wählen, sodass die stark zulässige Baumstruktur erhalten bleibt.
