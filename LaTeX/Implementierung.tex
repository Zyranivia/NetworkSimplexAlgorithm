...
Der Programmierteil dieser Bachelorarbeit implementiert den Netzwerk-Simplex-Algorithmus wie in \cref{ch:NSA} beschrieben, wobei einzelne Strukturen verändert wurden. Der Code ist in \cpp  geschrieben und hält sich an den \cppelf-Standard. Es folgt eine vollständige Übersicht der Codebestandteile, für das Gesamtkonstrukt siehe TODO Anhang. An einigen Stellen wurden für ein besseres Layout Variablennamen angepasst.
...
Die Klasse \lstinline|class Circle| speichert gerichtete Kreise als zwei Vektoren, die Kanten wie gehabt über Anfangsknoten, Endknoten und die \lstinline|bool isResidual| identifizieren. Außerdem speichern sie ihre jeweiligen Werte für $\delta$ und $c(\bar{C}_{T,e})$; diese müssen jedoch vom Algorithmus gesetzt werden. Nicht explizit in der Klassendeklaration schlägt sich die Invariante nieder, dass im Verlauf des Algorithmus jeder \lstinline|Circle| durch die erste Kante als einzige Nichtbaumkante spezifiziert wird.
...
Sei $W:=E(C_{T,e_1})\cap E(C_{T,e_2})$ der Schnitt der beiden ungerichteten Kreise unter $T$. Dann gilt:
...
void Circle::update(Circle& c) {
  ...
  //do second if-case and reverse back
  if (it != edges.end()) {
    c.rotateBy(0, true);
    reversed = true;
  }
  ...
  while (iSo != sources.end()) {
    if (std::abs(n.getNodes().find(*iSo)->second.b_value)
     <= std::abs(n.getNodes().find(*iSi)->second.b_value)){
      std::vector<size_t> temp = {*iSo, artNode, *iSi};
      n.addFlow(temp,
...
Für LC wissen wir zunächst nur, ob die Instanz lösbar ist oder nicht. Um erneut die Routine \lstinline|optimize| aufrufen zu können, müssen wir sicherstellen, dass der künstliche Knoten ein Blatt des aktuellen Baumes $T$ ist, da wir ihn sonst nicht einfach aus $T$ entfernen können.

Hierfür gibt es zwei Ansätze: Ein Weg ist es, einfach den künstlichen Knoten zu löschen und auf dem Graphen einen neuen Baum $T'$ zu bestimmen, der mit dem aktuellen Fluss $f$ eine zulässige Baumlösung $(T',f)$ bildet. In meiner Umsetzung führen wir dagegen degenerierte Iterationen durch, bis der künstliche Knoten ein Blatt ist. Dies schien mir der kohärente Ansatz zu sein.
